...
def test_get_winner(ruleset):
    assert ruleset.get_winner('SPOCK', 'SCISSORS') == 'SPOCK', "Logic failed: SPOCK must beat SCISSORS."
    assert ruleset.get_winner('ROCK', 'PAPER') == 'PAPER', "Logic failed: PAPER must beat ROCK."
    assert ruleset.get_winner('LIZARD', 'LIZARD') == False, "Logic failed: There shouldn't be any winner."
